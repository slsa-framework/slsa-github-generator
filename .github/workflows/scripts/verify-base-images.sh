...
find . \( ! -name vendor -o -prune \) \( ! -name node_modules -o -prune \) -type f -name Dockerfile -print0 | while IFS= read -r -d '' f; do
    ...
    grep "^FROM " "${f}" | while IFS= read -r line; do
        ...
        if [[ "${image_name}" == gcr.io/distroless/* ]]; then
            ...
            cosign verify --key .github/workflows/scripts/distroless.pub "$image_full" --certificate-oidc-issuer https://accounts.google.com --certificate-identity keyless@distroless.iam.gserviceaccount.com
        else
